...
public class SemanticListener extends MicroJavaBaseListener {
    ...
    /**
     * Observe que o construtor do listener (acima) j� empilha um ambiente na tabela
     * de s�mbolos, que vai ser associado ao ambiente dos identificadores globais do
     * programa sendo analisado.
     * 
     * Vamos come�ar pela declara��o de vari�veis simples. Observe na gram�tica
     * MicroJava.g4 as regras associadas:
     * 
     * varDecl : type IDENT (',' IDENT)* ';' ;
     * 
     * type: IDENT # TypeWithoutArray | IDENT '[' ']' # TypeWithArray ;
     * 
     * Vamos redefinir os m�todos que s�o executados ap�s visitar n�s da �rvore
     * constru�dos com as regras acima. Acompanhe os coment�rios e implemente o que
     * � solicitado.
     */

    /**
     * Para tipos simples, o c�digo abaixo j� est� completo. Procura o nome do tipo
     * na tabela e emite msgs de erro, caso necess�rio. Deixa na pilha um objeto
     * denotando o tipo encontrado. Procure entender o c�digo.
     */
    ...
    public void exitTypeWithArray(MicroJavaParser.TypeWithArrayContext ctx) {
        // TODO
        /**
         * Insira c�digo para declarar tipo com array, que � bem semelhante a tipo
         * simples. A �nica diferen�a para o m�todo anterior � que, no caso de n�o haver
         * nenhum erro, em vez de empilhar o tipo do s�mbolo, deve ser empihado um
         * TypeArray com esse tipo dentro dele. Trocar:
         * 
         * stack.push(s.getSemanticType());
         * 
         * pelo empilhamento de um TypeArray com s.getSemanticType() dentro dele. Para
         * criar esse tipo, use servi�o adequado da f�brica semanticFactory.
         */
        ...
        if (s == null) {
            emitErrorMessage(t, "tipo não declarado");
            stack.push(symbolTable.getNullType());
        } else if (!s.isClass()) {
            emitErrorMessage(t, "Identificador " + name + " não é um tipo válido");
    ...
    public void exitVarDecl(MicroJavaParser.VarDeclContext ctx) {
        /**
         * O tipo das var�aveis est� na pilha. N�s o empilhamos nos m�todos que acabamos
         * de ver. Esse tipo � desempilhado e colocado em st.
         */
        SemanticType st = (SemanticType) stack.pop();

        /**
         * O contexto � consultado usando o m�todo IDENT(), que neste caso cont�m uma
         * lista de identificadores que ser�o declarados. Por exemplo, em: int x, y, z;
     ...
     * arquivo input/teste01.txt. As impress�es em preto s�o um "debug" da tabela de
     * s�mbolos que pode ser desligado no arquivo Piloto. As impress�es em vermelho
     * s�o erros detectados. Alguns deles poder�o ser corrigidos � medida que formos
     * implementando os outros m�todos da an�lise.
     */

    // TODO
    /**
     * Em seguida, fa�a uma altera��o no m�todo acima para emitir msg de erro quando
     * uma vari�vel for declarada 2 vezes com mesmo nome. A msg deve identificar
     * qual vari�vel causa erro, com texto como:
     * 
     * "Identificador j� declarado: i"
     * 
     * Para isso, basta verificar o valor de s retornado. Os m�todos de inser��o de
     * s�mbolo na tabela retornam null quando n�o conseguem inserir um novo s�mbolo
     * - a causa esperada � que j� exista s�mbolo com esse nome no ambiente atual.
     * Observe que o m�todo de emitir mensagens de erro recebe como primeiro
     * par�metro o token onde o erro ocorreu. Neste caso, use tn.getSymbol().
     * 
     * Execute novamente o Piloto e veja as msgs de erro. Voc� ir� ter mais msgs de
     * erro que o desejado, mas isso � devido ao fato de que ainda n�o estamos
     * controlando o empilhamento de novos ambientes.
     */

    /**
     * Vamos agora analisar declara��es de constantes. As regras da gram�tica s�o:
     * 
     * constDecl: 'final' type IDENT '=' valConst ';';
     * 
     * valConst: INT_NUMBER # ConstInt | REAL_NUMBER # ConstReal | CHAR_CONST #
     * ConstStr ;
     * 
     * Voc� pode encontrar abaixo c�digo pronto para as 3 alternativas da regra
     * valConst.
     */
    ...
    /**
     * O c�digo abaixo analisa a declara��o de uma constante. Complete o c�digo
     * quando solicitado.
     */
    public void exitConstDecl(MicroJavaParser.ConstDeclContext ctx) {
        /**
         * Pelo menos 2 tipos dever�o estar empilhados em stack, e s�o coletados nas
         * vari�veis st2 (tipo do valor da constante) e st1 (tipo declarado no
         * programa). Por exemplo, em final int size = 10; st2 ser� TypeInteger por
         * causa de "10", e st1 ser� tamb�m TypeInteger por causa de "int".
         */
        ...
        if (!compatibleTypes(st1, st2)) {
            ...
            // TODO
            /**
             * Insira c�digo para adicionar a constante na tabela de s�mbolos. Use o m�todo
             * addSymbolConstant da tabela. Use tamb�m ctx.IDENT().getText() para obter o
             * nome da constante e ctx.valConst().getText() para obter o valor da constante.
             * Ap�s a opera��o de adicionar a constante, verifique se ela foi realizada com
             * sucesso. Caso contr�rio, emita msg de erro indicando identificador repetido.
             */
    ...
    /**
     * Rode o programa para testar declara��es de constantes. Insira no arquivo de
     * entrada alguns casos que verifiquem situa��es de erro, tais como: final int
     * size = 10; final int size = 20; // repeti��o de declara��o final int c = "a";
    ...
    /**
     * Antes de visitar a declara��o de uma classe, ou seja, antes de processar as
     * declara��es das vari�veis dentro da classe, as a��es abaixo devem ser
     * executadas. Sua tarefa � traduzir os trechos de pseudo-c�digo para Java.
     */
    ...
    /**
     * OBS: veja que o c�digo acima menciona 2 pilhas diferentes: - stack � a pilha
     * auxiliar que usamos para compartilhar informa��es entre os m�todos do
     * listener; - a tabela de s�mbolos � por si pr�pria uma pilha de ambientes.
     */

    /**
     * Ap�s visitar a declara��o de uma classe, o ambiente constru�do no topo da
     * tabela de s�mbolos cont�m as var��veis internas � classe (atributos). Esse
     * ambiente deve ser desempilhado da tabela de s�mbolos e associado ao tipo do
     * s�mbolo da nova classe (que empilhamos no m�todo anterior). O c�digo abaixo
     * est� completo, basta tirar os coment�rios.
     */
    ...
    /**
     * Ap�s a implementa��o dos 2 m�todos acima, rode novamente o Piloto e veja
     * altera��es no comportamento: - vari�veis internas a declara��es de classe n�o
     * mais conflitam oom vari�veis globais, mesmo se tiverem nomes iguais; -
     ...
     * 
     * 1) Inserir na tabela de s�mbolos um m�todo com tipo de retorno definido por
     * typeOrVoid (tipos dos par�metors ser�o definidos depois). 2) Antes de
     * analisar os par�metros, empilhar novo ambiente na tabela de s�mbolos. 3)
     * Analisar as declara��es dos par�metros como vari�veis locais, que ser�o
     * inseridas no novo ambiente. Al�m disso, inserir os tipos dos par�metros na
     * lista de par�metros do m�todo criado. 4) Continuar o processo, analisando as
     * vari�veis locais do m�todo. 5) Ap�s o t�rmino de toda a an�lise, desempilhar
     * o novo ambiente da tabela de s�mbolos.
     * 
     * Siga as instru��es abaixo para a correta implementa��o da an�lise sem�ntica
     * de declara��es de m�todos.
     */

    /**
     * Ao iniciar a decla��o de um m�todo, o token associado ao nome desse m�todo �
     * empilhado, simplesmente para poder ser usado no momento em que for inserida a
     * declara��o na tabela de s�mbolos. O c�digo abaixo est� completo.
     */
    ...
    public void exitTypeOrVoid(MicroJavaParser.TypeOrVoidContext ctx) {
    	//TODO: Traduza o pseudo-código para Java.
    ...
    /**
     * O c�digo abaixo est� completo, sendo executado logo antes de se iniciar a
     * an�lise dos par�metros do m�todo. Procure entender.
     */
    ...
    /**
     * O c�digo a seguir � executado para cada par�metro da declara��o de um m�todo.
     * Traduza o pseudo-c�digo quando encontrar "TODO".
     */
    public void exitFormPar(MicroJavaParser.FormParContext ctx) {
        ...
        SymbolMethod sm = symbolTable.addSymbolMethod(methodName.getText(), st);
        if (sm == null) {
            emitErrorMessage(methodName, "Identificador já declarado: " + methodName.getText());
            stack.push(null);
        } 
    ...
    /**
     * Ao terminar de analisar os par�metros, desempilhar o tipo do m�todo que
     * estava em stack.
     */
    ...
    /**
     * Ao terminar a declara��o de um m�todo, o ambiente � desempilhado da tabela de
     * s�mbolos.
     */
    ...
    /**
     * Insira no arquivo de entrada algumas declara��es de m�todos e rode o programa
     * piloto para testar. Lembre que a declara��o de vari�veis locais � um pouco
     * diferente de Java. Exemplo:
     * 
     * void main(char p) int x, k; { }
     * 
     * Procure testar situa��es de erro, como par�metros e vari�veis locais com
     * nomes repetidos.
     * 
     * Esta foi a �ltima tarefa deste tutorial. Parab�ns!
     */

    /**
     * ----------------------------------------------------------------------- As
     * fun��es abaixo j� est�o completamente implementadas e s�o auxiliares para a
     * implementa��o da an�lise sem�ntica.
     * -----------------------------------------------------------------------/
     * 
     * 
