...
/**
 * 
 * Tradução de assembly da máquina virtual MicroJava
 * para código binário da mesma máquina.
 * 
 * Uso:
 *   java Assembler arquivoFonte arquivoObjeto
 *   
 * Além das instruções definidas no documento de
 * especificação da máquina virtual MicroJava, o
 * assembly processado por este programa aceita:
 * 
 * - comentários em linhas começando com "#";
 * 
 * - instrução "data numVars", onde "numVars" é um
 * número inteiro positivo, para definir o tamanho
 * da área de dados globais (estáticos);
 * 
 * - instrução "label nomeLabel", para definição
 * e referência de pontos no código;
 * 
 * - uso de rótulos (labels) definidos em instruções
